...
- **Rerolls in the background.** With the game unfocused, the reset key is sent straight to its X11 window instead of through uinput, so auto-reroll carries on while you use something else. The game asks to be activated when that happens and the window manager has to say no: on KDE, add a window rule for window class `Megabonk.x86_64` (exact match, exact case) forcing *Focus stealing prevention* to *Extreme*. Without the rule the game grabs focus on every reroll; untick *Settings → Auto-Reroll → Keep rerolling when the game is not focused* (`RESET_WHEN_UNFOCUSED` in `config.json`) to get the old wait-for-focus behaviour back. Native build only so far.
